...
with graph.as_default():
	...
	fb = tv.Variable(tf.zeros[1, num_nodes])
	...
	def lstm_cell(i, o, state):
		# Create LSTM cell
		# Various connections b/w previous state and gates omitted
		# http://arxiv.org/pdf/1402.1128v1.pdf
		input_gate = tf.sigmoid(tf.matmul(i, ix) + tf.matmul(o, im) + ib)
		forget_gate = tf.sigmoid(tf.matmul(i, fx) + tf.matmul(o, fm) + fb)
		update = tf.mamtul(i, cx) + tf.matmul(o, cm) + cb
	...
	learning_rate = tf.train.exponential_decay(10.0, global_step, 5000,
		0.1, staircase=True)
